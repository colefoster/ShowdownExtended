...
TooltipPlus.showPokemonTooltip = function showPokemonTooltip(clientPokemon, serverPokemon, isActive, illusionIndex) {
 var _this3 = this;
  const pokemon = clientPokemon || serverPokemon;
    ...
    let pokeIconBuf = '<img src="' + Dex.resourcePrefix + 'sprites/gen5/' + BattleLog.escapeHTML((pokemon.speciesForme).replace(' ', '').replace('-', '')).toLowerCase()+'.png"/>';
    ...
    for (const moveid of serverPokemon.moves) {
      ...
      // Show move base power
        moveListBuf += moveName + ', Base power: ' + move.basePower + ' ' +
            ...
            (move.category === "Status" ? '' : (moveStr === 1 ? ` (x${moveStr})` : `<b> (x${moveStr})</b>`)) +
            '<br/>';
...
TooltipPlus.showMoveTooltip = function (move, isZOrMax, pokemon, serverPokemon, gmaxMove) 
    ...
    let defTypes = (this.battle.dex.species.get(foeActive[0].speciesForme).types);
            ...
            let moveStr = 1 * TooltipPlus.BattleTypeChart[defTypes[0]].damageGiven[moveType];

            if (defTypes.length > 1) {
                moveStr = moveStr * TooltipPlus.BattleTypeChart[defTypes[1]].damageGiven[moveType]
            }
